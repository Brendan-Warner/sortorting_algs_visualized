...
vector<int> merge(vector<int> to_sort_left, vector<int> to_sort_right, int max, bool output)
{
	...
	while (!to_sort_left.empty() && !to_sort_right.empty())//checks if one of the partions is empty.
	{
		if (to_sort_left[0] < to_sort_right[0])//if the left partion number is smaller, then add it to the vector, else we add the right number to the vector.
		{
			...
			print_array_merge(combined_vects,to_sort_left, to_sort_right, max, output);
...
vector<int> merg_sort(vector<int> to_sort, int max, bool output)
{
	if (to_sort.size() == 1)
	{
		return to_sort;
	...
	to_sort_left = merg_sort(to_sort_left, max, output);
	to_sort_right = merg_sort(to_sort_right, max, output);

	return merge(to_sort_left, to_sort_right, max, output);
...
void merge_sort_prep(int size, int output, bool file)
{
	...
	to_sort = merg_sort(to_sort, max_num, file);
